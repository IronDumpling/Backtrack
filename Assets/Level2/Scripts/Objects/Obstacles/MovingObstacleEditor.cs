...
{
    MovingObstacles m_MovingPlatform;
    ...
    {
        m_PreviewPosition = 0;
        m_MovingPlatform = target as MovingObstacles;

        if(!EditorApplication.isPlayingOrWillChangePlaymode)
            MovingObstaclePreview.CreateNewPreview(m_MovingPlatform);
    }

    private void OnDisable()
    {
        MovingObstaclePreview.DestroyPreview();
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUI.BeginChangeCheck();
        MovingObstacles.MovingPlatformType platformType = (MovingObstacles.MovingPlatformType)EditorGUILayout.EnumPopup("Looping", m_MovingPlatform.platformType);
        if (EditorGUI.EndChangeCheck())
        {
    ...
    private void OnSceneGUI()
    {
        ...
        for (int i = 0; i < m_MovingPlatform.localNodes.Length; ++i)
        {
            ...
            if (i == 0)
            {
                if (m_MovingPlatform.platformType != MovingObstacles.MovingPlatformType.LOOP)
                    continue;
    ...
    void MovePreview()
    {
        ...
        MovingObstaclePreview.preview.transform.position = m_MovingPlatform.transform.TransformPoint(localPos);
